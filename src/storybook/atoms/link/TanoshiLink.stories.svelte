<script lang="ts">
	...
	const internalLink = new TanoshiLinkModel('Open in current tab')
		.setLink('https://github.com/deozza/tanoshi')
		.setIsInternal(true);
	const externalLink = new TanoshiLinkModel('Open in new tab')
		.setLink('https://github.com/deozza/tanoshi')
		.setIsInternal(false);
</script>
...
<Meta
	...
	argTypes={{
		...
		content: {
			name: 'content',
			type: 'string',
			description: 'Text displayed by the link',
			control: false
		},
		link: {
			name: 'link',
			type: 'string',
			control: false,
			description: 'URl the link leads to'
		},
		isInternal: {
			name: 'isInternal',
			type: 'boolean',
			control: false,
			description: 'Set if the link should be opened in a new tab or not'
		}
	}}
/>
